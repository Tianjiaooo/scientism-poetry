...
router.get('/top', async (req, res) => {
  try {
    const authors = await Poem.aggregate([
      {
        $match: { status: 'published' }, // 仅统计已发布的诗歌
      },
      {
        $group: {
          _id: '$author', // 按作者 ID 分组
          poemCount: { $sum: 1 }, // 统计每个作者的诗歌数量
        },
      },
      {
        $sort: { poemCount: -1 }, // 按发布数量降序排序
      },
      {
        $limit: 10, // 只取前 10 位作者
      },
      {
        $lookup: {
          from: 'users', // 关联用户表
          localField: '_id', // Poem 表中的 author 字段
          foreignField: '_id', // User 表中的 _id 字段
          as: 'authorDetails', // 将用户信息存入 authorDetails
        },
      },
      {
        $unwind: '$authorDetails', // 解包 authorDetails 数组
      },
      {
        $project: {
          _id: 1,
